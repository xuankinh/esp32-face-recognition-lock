...
class FirebaseBridge:
    def __init__(self):
        ...
        self.on_delete_face    = None   # fn(name: str) — xóa 1 người
        self.on_delete_all     = None   # fn() — xóa toàn bộ vectors.npz
        self.on_door_status    = None   # fn(status: str)

        threading.Thread(target=self._clear_stale_lock, daemon=True).start()
        threading.Thread(target=self._poll_loop,  daemon=True).start()
        threading.Thread(target=self._log_worker, daemon=True).start()

    def _clear_stale_lock(self):
        try:
            requests.put(f"{config.FIREBASE_URL}/RobotLeTan/SessionLock.json", json="", timeout=3)
        except Exception:
            pass
    ...
    def _poll_loop(self):
        ...
        while True:
            try:
                r = requests.get(f"{config.FIREBASE_URL}/RobotLeTan.json", timeout=3)
    ...
    def _handle_command(self, data: dict):
        ...
        if cmd == "REGISTER_NEW_FACE" and self.on_register_face:
            ...
            # Xóa 1 người: ESP32 xóa thẻ, Python xóa vector theo tên
            name = data.get("name", "").strip()
            if name and self.on_delete_face:
                print(f"[FIREBASE] Xóa khuôn mặt: {name}")
                self.on_delete_face(name)

        elif cmd == "ADMIN_RESET":
            # Xóa toàn bộ: ESP32 đã xử lý NVS, Python xóa hết vectors.npz
            if self.on_delete_all:
                print("[FIREBASE] Xóa toàn bộ khuôn mặt!")
                self.on_delete_all()
    ...
    def _log_worker(self):
        while True:
            ...
            try:
                if "/DailyLogs/" in path:
                    requests.post(f"{config.FIREBASE_URL}{path}.json", json=payload, timeout=3)
                else:
                    requests.put(f"{config.FIREBASE_URL}{path}.json",  json=payload, timeout=3)
            except Exception:
                pass
